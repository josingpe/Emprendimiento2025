...
def mostrar_login(page):
    ...
    # Función para actualizar la tasa (ejecuta la tarea asíncrona en un hilo)
    def actualizar_tasa(e=None):
        loop = asyncio.get_event_loop()
        future = asyncio.run_coroutine_threadsafe(obtener_tasa_dolar(), loop)
        try:
            tasa = future.result()  
            tasa_text.value = f"Tasa USD/VES: {tasa}"
            page.update()
        except Exception as error:
            tasa_text.value = f"Error: {error}"
            page.update()
